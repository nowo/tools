...
function setCustomBackground(url) {
  ...
  // console.log(url);
  // $(".shadowboard, .clipboard ").css('background-image', 'url(' + url + ')')
...
function appendFigure(clip_path, shape) {
  ...
  $("figure:not(.disabled)").unbind().click(function () {
    ...
    if ($(this).hasClass("on") && type == 'custom') {
...
function setupDemo(coords, wid, hei) {
  // console.log(coords);
  width = wid || width
  height = hei || height
